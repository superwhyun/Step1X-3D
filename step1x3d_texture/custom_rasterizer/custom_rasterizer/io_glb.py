...
def get_attribute_data(gltf, accessor_index):
    ...
    byte_stride = (
        ...
        else num_components * np.dtype(dtype).itemsize
    )
...
def get_world_transform(gltf, node_index, parents, world_transforms):
    ...
    parent_transform = get_world_transform(
        gltf, parent_index, parents, world_transforms
    )
    ...
    for node_index, node in enumerate(gltf.nodes):
        if node.mesh is not None:
            ...
            for primitive in mesh.primitives:
                ...
                mode = (
                    primitive.mode if primitive.mode is not None else 4
                ...
                if "POSITION" in attributes and attributes["POSITION"] is not None:
                    ...
                    transformed_positions = (
                        world_transform @ positions_homogeneous.T
                    ).T[:, :3]
                    ...
                    if (
                        ...
                        if not image_index in images:
                            ...
                            image_data = get_image_data(
                                gltf, image, os.path.dirname(path)
                            )
